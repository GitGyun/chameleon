...
def configure_experiment(config, model, is_rank_zero=True):
    ...
    # parse precision
    precision = config.precision.strip('fp') if config.precision in ['fp16', 'fp32'] else config.precision
    precision = f'{precision}-true' if 'bf16' not in precision else precision
...
def set_strategy(config, precision):
    if config.strategy == 'ddp' and not config.single_gpu:
        ...
        strategy = SingleHPUStrategy(precision_plugin=HPUPrecisionPlugin(precision))
