...
class LightningTrainWrapper(pl.LightningModule):
    def __init__(self, config, verbose=True):
        ...
        self.n_devices = config.num_devices
    ...
    def forward(self, train_data):
        ...
        Y_S_in = Y_S * M_S + torch.ones_like(Y_S) * self.config.mask_value * (1-M_S)
    ...
    def custom_flip(self, tensor, direction):
        if isinstance(direction, (list, tuple)):
            new_tensor = tensor.clone()
            for _direction in direction:
                new_tensor = self.custom_flip(new_tensor, _direction)
        elif direction == -1:
            new_tensor = torch.zeros_like(tensor)
            for i in range(tensor.shape[-1]):
                new_tensor[..., tensor.shape[-1] -1 - i] = tensor[..., i].clone()
        
        elif direction == -2:
            new_tensor = torch.zeros_like(tensor)
            for i in range(tensor.shape[-2]):
                new_tensor[..., tensor.shape[-2] -1 - i, :] = tensor[..., i, :].clone()
        return new_tensor

    # hpu does not support autocasting
        ...
        for X_in_, channel_idxs in zip(X_in.split(channel_chunk_size, dim=1),
            ...
            for X_in__ in X_in_.split(chunk_size, dim=2):
                ...
                if getattr(self.config, "use_ninecrop", False):
                    ...
                    Y_pred_2 = self.custom_flip(self.model.predict_query(self.custom_flip(X_in__, -1), channel_idxs=channel_idxs, **kwargs), -1).sigmoid()
                
                    # Y_pred_3 = self.model.predict_query(X_in__.flip(-2), channel_idxs=channel_idxs, **kwargs).flip(-2).sigmoid()
                    Y_pred_3 = self.custom_flip(self.model.predict_query(self.custom_flip(X_in__, -2), channel_idxs=channel_idxs, **kwargs), -2).sigmoid()
                
                    # Y_pred_4 = self.model.predict_query(X_in__.flip((-1, -2)), channel_idxs=channel_idxs, **kwargs).flip((-1, -2)).sigmoid()
                    Y_pred_4 = self.custom_flip(self.model.predict_query(self.custom_flip(X_in__, (-1, -2)), channel_idxs=channel_idxs, **kwargs), (-1, -2)).sigmoid()
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if batch_idx == 0:
            ...
            vis_batch = to_device((X_vis, Y_vis, M_vis, Y_pred_vis, aux_vis), device='cpu', dtype=torch.float)
