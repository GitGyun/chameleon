...
class DAVIS(object):
    ...
    def __init__(self, root, task='unsupervised', subset='val', sequences='all', resolution='480p', codalab=False, year='2017'):
        ...
        if self.year == '2016':
            self.imagesets_path = os.path.join(self.root.replace('2016', '2017'), 'trainval', 'ImageSets', self.year)
    ...
    def _check_directories(self):
        if not os.path.exists(self.root):
            raise FileNotFoundError(f'DAVIS not found in the specified directory in {self.root}, download it from {self.DATASET_WEB}')
        if not os.path.exists(os.path.join(self.imagesets_path, f'{self.subset}.txt')):
            raise FileNotFoundError(f'Subset sequences list for {self.subset} not found in {self.imagests_path}, download the missing subset '
                                    f'for the {self.task} task from {self.DATASET_WEB}')
        if self.subset in ['train', 'val'] and not os.path.exists(self.mask_path):
            raise FileNotFoundError(f'Annotations folder for the {self.task} task not found in {self.mask_path}, download it from {self.DATASET_WEB}')
    ...
    def _get_all_elements(self, sequence, obj_type):
        ...
        if self.year == '2016' and obj_type != 'scores':
            all_objs /= 255.
