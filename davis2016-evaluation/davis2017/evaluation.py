...
class DAVISEvaluation(object):
    ...
    def _evaluate_semisupervised(all_gt_masks, all_res_masks, all_void_masks, metric):
        if all_res_masks.shape[-2:] != all_gt_masks.shape[-2:]:
            size = all_gt_masks.shape[-2:]
            B = all_res_masks.shape[0]
            x = rearrange(torch.from_numpy(all_res_masks).float(), 'B N H W -> (B N) 1 H W')
            x = T.Resize(size)(x)
            all_res_masks = rearrange(x, '(B N) 1 H W -> B N H W', B=B).bool().numpy()
    ...
    def evaluate(self, res_path, metric=('J', 'F'), debug=False):
        ...
        for seq in tqdm(list(self.dataset.get_sequences())):
            if seq not in os.listdir(res_path):
                continue
