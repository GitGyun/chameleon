...
class FreiHand(Dataset):
    ...
    KP_CLASSES = ['wrist', 'thumb1', 'thumb2', 'thumb3', 'thumb4',
    ...
    def __init__(self, path_dict, split, base_size=(256, 256), crop_size=(224, 224), 
        ...
        self.img_root = os.path.join(path_dict[self.NAME], 'training')
    ...
    def _load_image(self, imgId):
        ...
        img_path = file_name.replace('training', f'{self.img_root}')
        
        # open image file
        img = Image.open(img_path)
        if img.size != self.base_size:
            img = img.resize(self.base_size)
        img = self.toten(img)
        if len(img) == 1:
...
class FreiHandBaseDataset(FreiHand):
    def __init__(self, *args, coco=None, **kwargs):
        super().__init__(*args, **kwargs)
        if coco is not None:
            self.coco = coco
        else:
            self.coco = SingletonCOCOFactory.get_coco(os.path.join(self.ann_root, f'freihand_{self.split}.json'))
...
class FreiHandBaseTrainDataset(FreiHandBaseDataset):
...
class FreiHandBaseTestDataset(FreiHandBaseDataset):
...
class FreiHandContinuousTrainDataset(FreiHandBaseTrainDataset):
...
class FreiHandContinuousTestDataset(FreiHandBaseTestDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.initialize_keypoints()


class FreiHandCategoricalDataset(FreiHandBaseDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.initialize_keypoints()
     

class FreiHandCategoricalTrainDataset(FreiHandCategoricalDataset):
    def __init__(self, label_augmentation, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class FreiHandCategoricalTestDataset(FreiHandCategoricalDataset):
