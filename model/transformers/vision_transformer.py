...
from .helpers import build_model_with_cfg, resolve_pretrained_cfg, named_apply, adapt_input_conv, resize_pos_embed
from timm.models.layers import DropPath, trunc_normal_, lecun_normal_
from .registry import register_model
from .custom_layers import Linear, LayerNorm, Mlp, PatchEmbed
...
default_cfgs = {
    ...
    'vit_large_patch16_224_512': _cfg(
        input_size=(3, 224, 512), crop_pct=1.0),
    'vit_large_patch16_160_384': _cfg(
        input_size=(3, 160, 384), crop_pct=1.0),
    'vit_large_patch16_224_384': _cfg(
        input_size=(3, 224, 384), crop_pct=1.0),
    'vit_large_patch16_128_224': _cfg(
        input_size=(3, 128, 224), crop_pct=1.0),
    'vit_large_patch16_320': _cfg(
        input_size=(3, 320, 320), crop_pct=1.0),
    'vit_large_patch16_512': _cfg(
        input_size=(3, 512, 512), crop_pct=1.0),


    'vit_large_patch14_224': _cfg(url=''),
    'vit_large_patch14_392': _cfg(url='', input_size=(3, 392, 392)),
    'vit_large_patch14_448': _cfg(input_size=(3, 448, 448), crop_pct=1.0, url=''),
    'vit_huge_patch14_224': _cfg(url=''),
    'vit_giant_patch14_224': _cfg(url=''),
    'vit_gigantic_patch14_224': _cfg(url=''),
    'vit_large2_patch14_224': _cfg(url=''),
    ...
    'vit_large_patch14_dinov2.lvd142m': _cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 518, 518), crop_pct=1.0),
    'vit_large_patch14_dinov2_224': _cfg(
        url='https://dl.fbaipublicfiles.com/dinov2/dinov2_vitl14/dinov2_vitl14_pretrain.pth',
        hf_hub_id='timm/',
        license='apache-2.0',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 224, 224), crop_pct=1.0),
    'vit_large_patch14_dinov2_392': _cfg(
        url='https://dl.fbaipublicfiles.com/dinov2/dinov2_vitl14/dinov2_vitl14_pretrain.pth',
        hf_hub_id='timm/',
        license='apache-2.0',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 392, 392), crop_pct=1.0),
    'vit_large_patch14_dinov2_448': _cfg(
        url='https://dl.fbaipublicfiles.com/dinov2/dinov2_vitl14/dinov2_vitl14_pretrain.pth',
        hf_hub_id='timm/',
        license='apache-2.0',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
    ...
    'vit_large_patch14_reg4_dinov2.lvd142m': _cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 518, 518), crop_pct=1.0),
    'vit_large_patch14_reg4_dinov2_224': _cfg(
        url='https://dl.fbaipublicfiles.com/dinov2/dinov2_vitl14/dinov2_vitl14_reg4_pretrain.pth',
        hf_hub_id='timm/',
        license='apache-2.0',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 224, 224), crop_pct=1.0),
    'vit_large_patch14_reg4_dinov2_392': _cfg(
        url='https://dl.fbaipublicfiles.com/dinov2/dinov2_vitl14/dinov2_vitl14_reg4_pretrain.pth',
        hf_hub_id='timm/',
        license='apache-2.0',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 392, 392), crop_pct=1.0),
    'vit_large_patch14_reg4_dinov2_448': _cfg(
        url='https://dl.fbaipublicfiles.com/dinov2/dinov2_vitl14/dinov2_vitl14_reg4_pretrain.pth',
        hf_hub_id='timm/',
        license='apache-2.0',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
    ...
    'vit_large_patch14_448_clip_laion2b': _cfg(
        hf_hub_id='laion/CLIP-ViT-L-14-laion2B-s32B-b82K',
        hf_hub_filename='open_clip_pytorch_model.bin',
    ...
    'vit_giant_patch14_224_clip_laion2b': _cfg(
        ...
        hf_hub_filename='open_clip_pytorch_model.bin',
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD, num_classes=1024),
    'vit_large_patch14_392_clip_laion2b': _cfg(
        hf_hub_id='laion/CLIP-ViT-L-14-laion2B-s32B-b82K',
        hf_hub_filename='open_clip_pytorch_model.bin',
        mean=IMAGENET_INCEPTION_MEAN, std=IMAGENET_INCEPTION_STD, num_classes=768,
        input_size=(3, 392, 392)),

}


class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., n_bias_sets=0):
        super().__init__()
        ...
        self.qkv = Linear(n_bias_sets, dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = Linear(n_bias_sets, dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

    def forward(self, x, b_idx=None):
        B, N, C = x.shape

        qkv = self.qkv(x, b_idx=b_idx).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        # determine number of bias sets for norm layers
        super().__init__()
        self.norm1 = norm_layer(n_bias_sets, dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, n_bias_sets=n_bias_sets)
        self.ls1 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        self.norm2 = norm_layer(n_bias_sets, dim)
        self.mlp = mlp_layer(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop, n_bias_sets=n_bias_sets)
    ...
    def forward(self, x, b_idx=None):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x, b_idx), b_idx=b_idx)))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x, b_idx), b_idx=b_idx)))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        num_patches = self.patch_embed.num_patches * self.n_input_images
        self.img_size = img_size
        self.patch_size = patch_size

        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if class_token else None
        self.register_tokens = nn.Parameter(torch.zeros(1, reg_tokens, embed_dim)) if reg_tokens else None
        ...
        self.latent_size = 0
        self.n_input_images = config.n_input_images
    ...
    def forward_features(self, x, b_idx=None, feature_idxs=None, **kwargs):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in feature_idxs:
                if x.size(1) >= 2*self.num_patch_tokens:
                    ...
                    features.append(x[:, -self.num_patch_tokens:]) # remove cls toke
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    if pos_embed_w.shape != model.pos_embed.shape:
        pos_embed_w = resize_pos_embed(pos_embed_w, model.pos_embed.size(1),
    ...
    for name, p in model.named_parameters():
        if (name[-4:] == 'bias' and
            ...
            p.ndim == 2 and
            w[name].ndim == 1):
            assert w[name].shape[0] == p.shape[1]
            w[name] = repeat(w[name], 'd -> T d', T=p.shape[0])
...
def vit_base_patch16_224_mae(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_mae', pretrained=pretrained, **model_kwargs)
    return model
...
def vit_large_patch16_224_mae(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_224_mae', pretrained=pretrained, **model_kwargs)
    return model
...
def vit_large_patch16_512(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_512', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_large_patch16_224_512(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_224_512', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_large_patch16_160_384(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_160_384', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_large_patch16_224_384(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_224_384', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_large_patch16_128_224(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_128_224', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_large_patch16_320(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_320', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def vit_large_patch14_392(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    """
    model_kwargs = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch14_392', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_large_patch14_448(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    """
    model_kwargs = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch14_448', pretrained=pretrained, **model_kwargs)
    return model
...
def vit_huge_patch14_224_mae(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_huge_patch14_224_mae', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def vit_large_patch14_dinov2_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=224)
    model = _create_vision_transformer(
        'vit_large_patch14_dinov2_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_large_patch14_dinov2_392(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=392)
    model = _create_vision_transformer(
        'vit_large_patch14_dinov2_392', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_large_patch14_dinov2_448(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=448)
    model = _create_vision_transformer(
        'vit_large_patch14_dinov2_448', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_large2_patch14_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1536, depth=24, num_heads=24, init_values=1e-5, img_size=224)
    model = _create_vision_transformer(
        'vit_large2_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
def vit_large_patch14_reg4_dinov2_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-L/14 for DINOv2 w/ 4 registers
    """
    model_args = dict(
        patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5,
    ...
    model = _create_vision_transformer(
        'vit_large_patch14_reg4_dinov2_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_large_patch14_reg4_dinov2_392(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-L/14 for DINOv2 w/ 4 registers
    """
    model_args = dict(
        patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5,
    ...
    model = _create_vision_transformer(
        'vit_large_patch14_reg4_dinov2_392', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_large_patch14_reg4_dinov2_448(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-L/14 for DINOv2 w/ 4 registers
    """
    model_args = dict(
        patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5,
    ...
    model = _create_vision_transformer(
        'vit_large_patch14_reg4_dinov2_448', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
def vit_large_patch14_448_clip_laion2b(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    Pretrained weights from CLIP image tower trained on LAION-2B image-text pairs.
    """
    model_kwargs = dict(
        patch_size=14, embed_dim=1024, depth=24, num_heads=16, pre_norm=True, norm_layer=nn.LayerNorm, **kwargs)
    model = _create_vision_transformer('vit_large_patch14_448_clip_laion2b', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def vit_large_patch14_392_clip_laion2b(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    Pretrained weights from CLIP image tower trained on LAION-2B image-text pairs.
    """
    model_kwargs = dict(
        patch_size=14, embed_dim=1024, depth=24, num_heads=16, pre_norm=True, norm_layer=nn.LayerNorm, **kwargs)
    model = _create_vision_transformer('vit_large_patch14_392_clip_laion2b', pretrained=pretrained, **model_kwargs)
    return model
